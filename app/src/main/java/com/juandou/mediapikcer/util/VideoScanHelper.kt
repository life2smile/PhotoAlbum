...
object VideoScanHelper {
    ...
    private fun generateThumbNail(filePath: String?): String? {
        filePath.isNullOrEmpty().let {
            var out: FileOutputStream? = null
            try {
                val folderArr: List<String> = filePath!!.split(File.separator)
                val fileName = folderArr.isNotEmpty().let {
                    folderArr[folderArr.size - 1]
                }
                val folder = File(MEDIA_PICK_TEMP_FOLDER_PATH)
                folder.exists().not().let {
                    folder.mkdirs()
                }
                val file = File(MEDIA_PICK_TEMP_FOLDER_PATH + fileName)
                file.exists().not().let {
